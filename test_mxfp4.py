...
#input_tensor[0,  0:32].mul_(32)
#input_tensor[0, 32:64].div_(8)
#input_tensor[1,  0:32].mul_(0)

quantized_data, scale = MXFP4QuantizeUtil.quantize(input_tensor)

recovered_tensor = MXFP4QuantizeUtil.dequantize(quantized_data, torch.bfloat16, scale)

#print("### input_tensor\n", input_tensor, input_tensor.size())
#print("\n### quantized_data\n", quantized_data, quantized_data.size())
#print("\n### scale\n", scale, scale.size())
#print("\n### recovered_tensor\n", recovered_tensor)


def test_single_gemm(M, N, K, dtype, has_bias=False):

    scale_factor = 1e-2

    A = torch.randn((M, K), dtype=dtype) / 10
    B = (torch.rand((N, K), dtype=dtype) - 0.5) * 2
    B = B * scale_factor

    Bq, Bs = MXFP4QuantizeUtil.quantize(B)
    Bdq = MXFP4QuantizeUtil.dequantize(Bq, dtype, Bs)

    B_packed = convert_weight_packed(Bq)
    Bs_packed = convert_scale_packed(Bs)
