...
def run_single_benchmark(processor, min_size, max_size, nwarmup, niters, batch_size, validate):

    print("\n### Run image preprocessor benchmark with {:d} images".format(batch_size))
    ...
    print("  Average time : {:.3f} ms".format(avg_time))
    print("  Total time   : {:.3f} ms".format(total_time))

    if validate:
        image_processor_torch = qwen2vl_preprocess_torch(images)

        atol, rtol = 1e-5, 1e-5
        res = torch.allclose(image_processor['pixel_values'], image_processor_torch['pixel_values'], rtol=rtol, atol=atol)
        print("")
        print("### validating: image_processor['pixel_values']: ", res)
        res2 = torch.equal(image_processor['image_grid_thw'], image_processor_torch['image_grid_thw'])
        print("### validating: image_processor['image_grid_thw']: ", res2)


def benchmark_image_preprocessor(processor, min_size, max_size, nwarmup, niters, batches, profile, validate):
    for batch_size in batches:
        with torch.autograd.profiler.profile(enabled=profile) as prof:
            run_single_benchmark(processor, min_size, max_size, nwarmup, niters, batch_size, validate)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batches", type=int, nargs='+', default=[5])
